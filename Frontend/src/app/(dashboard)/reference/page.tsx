  ...
  if (!syllabus) {
    return (
      <div className="mx-auto flex max-w-xl flex-col items-center gap-5 px-5 py-24 text-center">
        ...
        <div>
          <h1 className="font-display text-xl font-bold text-ink">No syllabus yet</h1>
  ...
  return (
    <div className="mx-auto max-w-xl px-5 py-12">
      <div className="mb-8 text-center">
        <h1 className="font-display text-2xl font-bold text-ink">Reference material</h1>
      ...
      {!materialsLoading && materials.length > 0 && (
        <div className="mb-6 rounded-2xl border border-border bg-surface p-4">
          <h2 className="mb-3 text-[12px] font-bold uppercase tracking-widest text-ink-3">
            ...
            {materials.map((m) => (
              <div key={m.id} className="flex items-center gap-3 rounded-lg border border-border px-3 py-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-ink-3">{m.chunks_indexed} chunks indexed</p>
                </div>
              </div>
            ))}
          </div>
          ...
          >
            {state === "file_selected" && file ? (
              <>
                ...
                <button onClick={(e) => { e.stopPropagation(); reset(); }} className="text-xs text-ink-3 hover:text-ink-2">
                  Remove
                </button>
              </>
            ) : (
              <>
                ...
                <div>
                  <p className="font-medium text-ink">Drop a reference PDF here</p>
      ...
      {state === "parsing" && (
        <div className="rounded-2xl border border-border bg-surface p-8 text-center">
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              <p className="font-medium text-ink">Reading and indexing {file?.name}…</p>
              <p className="mt-1 text-xs text-ink-3">This can take a minute for longer PDFs.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ── Success ────────────────────────────────────────────── */}
      {state === "success" && (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-emerald-200 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/10 p-8 text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-emerald-100">
            <CheckCircle2 size={22} className="text-emerald-500" />
          </div>
          <p className="font-medium text-ink">Indexed successfully</p>
        </div>
      )}
