    ...
    <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-4">
      ...
      <div className="rounded-xl border border-border bg-surface p-4">
        <div className="flex items-center gap-2 text-orange-500">
          <Flame size={16} />
          <span className="font-display text-xl font-bold text-ink">{data.streak_days}</span>
        </div>
        <p className="mt-0.5 text-[12px] font-medium text-ink">Day streak</p>
        <p className="text-[11px] text-ink-3">{data.streak_days > 0 ? "Keep it going" : "Answer a question today"}</p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface p-4">
        ...
        <p className="mt-0.5 text-[12px] font-medium text-ink">Today&apos;s goal</p>
      ...
      <div className="rounded-xl border border-border bg-surface p-4">
        ...
        <p className="mt-0.5 text-[12px] font-medium text-ink">Overall accuracy</p>
        <p className="text-[11px] text-ink-3">{data.total_attempts} question{data.total_attempts === 1 ? "" : "s"} answered</p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface p-4">
        <div className="flex items-center gap-2 text-amber-500">
          <AlertCircle size={16} />
          <span className="font-display text-xl font-bold text-ink">{data.weak_topics.length}</span>
        </div>
        <p className="mt-0.5 text-[12px] font-medium text-ink">Weak topic{data.weak_topics.length === 1 ? "" : "s"}</p>
      ...
      {(data.weak_topics.length > 0 || data.upcoming_revisions.length > 0) && (
        <div className="col-span-full grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <ul className="flex flex-col gap-1.5">
                {data.weak_topics.map((t) => (
                  <li key={t.topic_id} className="flex items-center justify-between text-[13px]">
                    <Link href={`/study/${t.topic_id}`} className="text-ink hover:text-brand-600 truncate">
                      {t.topic_name}
                    </Link>
          ...
          {data.upcoming_revisions.length > 0 && (
            <div className="rounded-xl border border-brand-200 dark:border-brand-500/30 bg-brand-50/50 dark:bg-brand-500/10 p-4">
              ...
              <ul className="flex flex-col gap-1.5">
                {data.upcoming_revisions.map((r) => (
                  <li key={r.topic_id} className="flex items-center justify-between text-[13px]">
                    <Link href={`/study/${r.topic_id}`} className="text-ink hover:text-brand-600 truncate">
                      {r.topic_name}
                    </Link>
                    <span className={`ml-2 shrink-0 text-[11px] font-semibold ${r.overdue ? "text-red-500" : "text-ink-3"}`}>
